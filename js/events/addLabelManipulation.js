...
export function addLabelManipulation(listen, state) {
  ...
  listen("mouseup", "", (e) => {
    ...
    if (from !== "" && to !== "" && from !== to) {
      ...
      const toNodeObj = state.graph.getNode(toNode).data;

      const toNodePortObj = state.graph.getNode(toNode).data.ports[toPort];

      const fromNodePortObj =
        state.graph.getNode(fromNode).data.ports[fromPort];

      const toAcceptableLinkTypes = [];
      const fromAcceptableLinkTypes = [];

      state.componentsLibrary.links.forEach((link) => {
        if (
          link.ports
            .map((p) => p.type)
            .some((type) => toNodePortObj.type === type)
        ) {
        ...
        if (
          link.ports
            .map((p) => p.type)
            .some((type) => fromNodePortObj.type === type)
        ) {
